...
{
    [UIView animateWithDuration:_touchEndAnimationDuration delay:0.f options:UIViewAnimationOptionCurveEaseOut animations:^{
        self.alpha = 0.0f;
...
- (id)initWithPoint:(CGPoint)point color:(UIColor *)color touchEndAnimationDuration:(NSTimeInterval)touchEndAnimationDuration touchEndTransform:(CATransform3D)touchEndTransform
{
    const CGFloat kFingerRadius = 34.f;//22.0f;
    
    if ((self = [super initWithFrame:CGRectMake(point.x-kFingerRadius, point.y-kFingerRadius, 2*kFingerRadius, 2*kFingerRadius)])) {
        self.opaque = NO;
        [color retain];
        self.layer.borderColor = [color colorWithAlphaComponent:0.45f].CGColor;
        self.layer.cornerRadius = kFingerRadius;
        self.layer.borderWidth = 1.0f;
        self.layer.backgroundColor = [color colorWithAlphaComponent:0.24f].CGColor;
        [color release];
...
//        _touchColor = [UIColor colorWithRed:0.251f green:0.424f blue:0.502f alpha:1.0f];
        self.touchColor = [UIColor colorWithWhite:.1f alpha:1.f];
        _touchEndAnimationDuration = 0.2f;
        _touchEndTransform = CATransform3DMakeScale(.7, .7, 1);
